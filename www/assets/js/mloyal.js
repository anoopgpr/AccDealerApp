...
var SERVER1 = "https://atootbandhan.mloyalretail.com/";
...
/*var SERVER = "http://localhost:8086/https://atootbandhan.mloyalretail.com/m/";
var SERVER1 = "http://localhost:8086/https://atootbandhan.mloyalretail.com/";
var SERVER2 = "http://localhost:8086/http://localhost:8086/https://atootbandhan.mloyalretail.com/APIs/";
var SERVER3 = 'http://localhost:8086/https://atootbandhan.mloyalretail.com/Rewards/';
var GSERVER2 = "http://localhost:8086/https://atootbandhan.mloyalretail.com/APIs/";
var GSERVER22 = "http://localhost:8086/https://atootbandhan.mloyalretail.com/APIs/";*/
...
$("#form").on('submit',(function(e) {
  e.preventDefault();
  $.ajax({
         url: "https://taghash.co/Atoot-Bandhan-Dealer/upload.php",
   type: "POST",
    ...
    else
    {
     ...
     $("#wait").css("display","none");
                ...
                $( function() {
					 ...
					 $("#site_new_popup").enhanceWithin().popup();
				});
				...
				{
					   if($.mobile.activePage.is('#page-forgot-password-dealer'))
					   {
					   ...
					   else 
					   {
						...
						if(dealeruser!='' && dealerpass!='')
	                    {
	                           ...
	                           $('#dealerusername').val(localStorage.getItem("usernamemloyal_atootbandhan"));
...
{
	$('#name').val($('#update_kyc_doc_type_de').val()+'_'+$('#update_mobile_de').val());
...
function checkapp_Version()
{
     ...
     $.getJSON(
            'https://taghash.co/atootbandhan/getapp_version.php', 
			{
...
function tx_Submit_old()
{
	
	var tdate = $('#tx_date').val();

	var mno = $('#tx_smobile').val();
	...
	var ihbmobile = $('#tx_ihbmobile').val();
	var ihbname = $('#tx_ihbname').val();
	var address = $('#tx_address').val();
	var ihbpincode = $('#tx_ihbpincode').val();
	var rate = $('#tx_rate').val();
	var billamt = $('#tx_billamt').val();
	var buildarea = $('#tx_buildarea').val();
	var dealercode = store.storecode;

	var product = $('input[name="tx_product"]:checked').val();
	console.log(product);
	...
	{
		toast('Please check product');
	}
	else if(quantity=='')
	{
		toast('Please check quantity');
	}
	else if(dealercode=='' || dealercode==null || dealercode=='undefined' || dealercode=='null')
	{
	...
	else
	{
		...
		$.ajax({
		    ...
		    data:{
							...
							"bill_amount":billamt,
							// "SiteType":,
							"BuildupArea":buildarea,
							"IHB_MobileNo":ihbmobile,
							"IHB_Name":ihbname,
							"IHB_Address":address,
							"IHB_PinCode":ihbpincode,
							"rate":rate
			...
			success: function(data, textStatus, xhr) {
				...
				{
					toast_p("Transaction request has been submitted for approval!");
...
function tx_Submit()
{
	...
	// var rate = $('#tx_rate').val();
	var product = $('input[name="tx_product"]:checked').val();
	var qunttype = product.split('|');
	console.log(product);


	//$('#tx_quantity_type').val(qunttype[1]);
	//var productType = $('#tx_quantity_type').val();
	var productType = qunttype[1];
	console.log(productType);
	if(productType == 'Bag'){
	var quantity = $('#tx_quantity').val();
	}else{
	var quantity = $('#tx_quantityLiquid').val();
	}
	...
	{
		checkfor_siteValidation(mno);
	}
}

function checkfor_siteValidation(mobileno)
{
	var ihbmobile = $('#tx_ihbmobile').val();
	var ihbname = $('#tx_ihbname').val();
	var ihbaddress = $('#tx_address').val();
	var ihbpincode = $('#tx_ihbpincode').val();
	var contractType = $('#tx_matrialstatus').val();
	 $.ajax({
			   ...
			   success: function(data, textStatus, xhr) {	
					   ...
					   if(data.sms.length>=1)
					   {
						   ...
						   if(data.sms[0].ContractorNO==mobileno)
						   {
								tx_Submit_2();
						   }
						   else
					   ...
					   {
							add_new_site(ihbmobile,ihbname,ihbaddress,mobileno,contractType,ihbpincode);
					   }
			   },
			   error: function(xhr, textStatus, errorThrown) {
...
function add_new_site(ihbmobile,ihbname,ihbaddress,cmobileno,contractType,ihbpincode)
{
	$.ajax({
    ...
    data: {
				...
				'district': '',
				'pincode': ihbpincode,
				'matrialstatus' : contractType
					
		  },
   
    success: function(data, textStatus, xhr) {
                 console.log("data:" + JSON.stringify(data));
                 console.log('Site Registration submitted.');
		...
		$.ajax({
			...
			success: function(data, textStatus, xhr) {
				...
				else
				{
					...
					update_sites_contractor(ihbmobile,mno);
...
function update_sites_contractor(ihbmobile,cmobile)
{
	$.ajax({
				url: SERVER2+'get_ihb_customer_details.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14',
				type: 'GET',
				timeout: 50000,
				dataType: 'json',
				data: { 'mobileno': ihbmobile,'contractorno': cmobile},
			   success: function(data, textStatus, xhr) {	
...
$(document).on('pageshow', '#addtransactions', function (event, ui) {
	...
	$('input[type=radio][name=tx_product]').change(function() {
						
		// var p_type=$("#tx_product :radio:checked").val();
		var p_type=$('input[name="tx_product"]:checked').val();
		// 
		var qunttype = p_type.split('|');
		console.log(qunttype);
		$('#tx_quantity_type').val(qunttype[1]);
		// document.getElementById("selectproname").innerHTML= qunttype[2];
		// console.log(qunttype[0]);
		// $('#tx_selectproduct').val(qunttype[0]);
		//   if(qunttype[1] == 'Liquid'){
		// 	$("#productliq_div").css('display', 'block');
		// 	$("#productbag_div").css('display', 'none');
		// }else{
		// 	$("#productliq_div").css('display', 'none');
		// 	$("#productbag_div").css('display', 'block');
		// }
	});
});
    ...
    success: function(data, textStatus, xhr) {
     ...
     console.log('data'+JSON.stringify(data));
		...
		else
		{
			...
			store.storecode = data.SEdata[0].SE_code ;
			store.store_name = data.SEdata[0].SE_name;
			//store.storemobile = data.SEdata[0].SE_mobile;

			document.getElementById("user_id_se").innerHTML='CSO Code '+store.storecode;
    		//document.getElementById("user_id1_se").innerHTML='SE Code '+store.storecode;
			document.getElementById("profilenm_se").innerHTML= store.store_name;
			document.getElementById("profilenm1_se").innerHTML= store.store_name;
...
function receivedEvent(id) {
 ...
 var push = PushNotification.init({ "android": {"senderID": "521174529015"}});
...
function showPoP_new(msg){
    ...
    //create a title for the popup
    $("<div />", {
        html: "<span style=\"padding-left: 31px;\">"+msg+"</span> <div class=\"ui-grid-a mt-30 mb-20\">"+
        "<div class=\"ui-block-solo\" style=\"padding: 0 10px\"> <a href=\"#homepage\" class=\"btn btn_primary\" id=\"popnot\">OK</a> </div>"+
      "</div>"
    }).appendTo($popUp);
...
function rateApp()
{
	var deviceType = (navigator.userAgent.match(/iPad/i))  == "iPad" ? "iPad" : (navigator.userAgent.match(/iPhone/i))  == "iPhone" ? "iPhone" : (navigator.userAgent.match(/Android/i)) == "Android" ? "Android" : (navigator.userAgent.match(/BlackBerry/i)) == "BlackBerry" ? "BlackBerry" : "null";
		 if (deviceType=='iPad' || deviceType=='iPhone') 
			 ...
			 {
               window.open("market://details?id=com.mobiquest.atootbandhan","_system");
...
function showInstagram_1()
{
	var ref = window.open('https://www.instagram.com/acclimited/', '_blank');
}
...
//get_item_master();
	...
	var d = new Date();
	var dd1 = d.getDate()-7;
	var mm1 = d.getMonth()+1; //January is 0!
    ...
    var sdate = yyyy1+'-'+mm1+'-'+dd1;


	document.getElementById("tx_date").value = today;


	document.getElementById("tx_date").setAttribute("max", today);
	document.getElementById("tx_date").setAttribute("min", sdate);

	//showDistrictList_new();
...
function showDistrictList_new()
{
  
  $.ajax({
    url: SERVER2+'get_areaoffice_district_api.asp?AOCODE='+user.ao_code,
    type: 'GET',
    timeout: 50000,
    dataType: 'json',
    // data: {
    ...
    success: function(data, textStatus, xhr) {
		...
		for (var i = 0; i < data.length; i++)
		{
				listItems+= "<option value='" + data[i].District_Code + "'>" + data[i].District_Name + "</option>";
		}
				  $("#tx_district").html(listItems);
      },
      error: function(xhr, textStatus, errorThrown) {
    }
  });
...
function fetchDistrictlist_sreg()
{
			
  var stateid = $('#sreg_state').val();
...
function EnroledContractor()
{
	...
	var rcount = checkforundefined($('#update_runningsite_count').val());
		...
		else if(district == null || district == 'undefined' || district == '')
		{       
			toast('Please enter mandatory fields.');
		}
		else if(city == '' || city == 'undefined' || city == '')
		{
			toast('Please enter mandatory fields.');
		}
		else if(mandate)
	      ...
	      $.ajax({
			...
			timeout: 30000,
		  	data: {
				...
				'dobmonth': mm,
				'dobyear': yyyy,
				'scode': store.storecode,
                'deviceType': deviceType,
				'doaday': '',
				'doamonth': '',
				'doayear': '',
				'storecd': store.storecode,
				'source': 'dealer',
				'point_claim': '',
				'floor_flat': address,
...
function addtxns_pg()
{
	$.mobile.changePage( "#addtransactions", { transition: "none"} );
}

function reset_enrollProfile_bydealer()
{
...
function getEnroledcontractor(){
	// console.log(user.mobile);
	var scode = localStorage.getItem('scode');
	// $.getJSON(
	// 	GSERVER2+'get_associated_member_by_store_json_api.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&store_code='+user.mobile, 
		$.ajax({
			url: SERVER2+'get_associated_member_by_store_json_api.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&store_code='+store.storecode,
			type: 'GET',
			timeout: 30000,
			success: function(data, textStatus, xhr) {
				// console.log(data);
				...
				$.mobile.changePage( "#enroleContractorList", {transition: "flip"} );
				toast('Your profile has been updated successfully.');
				for (let r = 0; r < obj.data.length; r++) {
					var fname = obj.data[r].FirstName;
					var lname = obj.data[r].LastName;
					var remobile = obj.data[r].Mobileno;
				}
						localStorage.setItem("profiledone_atootbandhan","Yes");
                         var name = fname+' '+lname;
						document.getElementById("profilenm").innerHTML = fname+' '+lname;
						document.getElementById("profilenm1").innerHTML = fname+' '+lname;

						if(user.firstname != '' || user.lastname != ''){
							var fn = fname.substring(0, 1);
							var ln = lname.substring(0, 1);
						}
						document.getElementById("sortname").innerHTML= fn+''+ln;
...
function showproDistrictList()
{
  var pstateid = $('#update_state_de').val();
  $.ajax({
    url: SERVER2+'Get_alldistrict_json_api.asp?stateid='+pstateid,
    type: 'GET',
    timeout: 50000,
    dataType: 'json',
...
function showproDistrictCityList()
{
	console.log('saurabh city');
  var pdistrictid = $('#update_district_de').val();
  $.ajax({
    url:SERVER2+'Get_allcity_json_api.asp?districtid='+pdistrictid,
    type: 'GET',
    timeout: 50000,
    dataType: 'json',
...
function getPending_members_byse()
{
	  ...
	  $.ajax({
		...
		timeout: 50000,
		dataType: 'json',
		data: {'apiuid': 'MLOYALAPI','apipswd':'Ml0yalAP!2o14','se_code': store.storecode },
		complete: function(xhr, textStatus) {
	   //called when complete
		}, success: function(data, textStatus, xhr) {
...
function getActive_members_byse()
{
	  ...
	  $.ajax({
		...
		timeout: 50000,
		dataType: 'json',
		data: {'apiuid': 'MLOYALCKBAPI','apipswd':'Ml0yalCKBAP!2!','se_code': store.storecode },
		complete: function(xhr, textStatus) {
	   //called when complete
		}, success: function(data, textStatus, xhr) {
			...
			success: function(data, textStatus, xhr) {
				 ...
				 else
				 {
					 for (var i = 0; i < data.data.length; i++)
					 {
						 ...
						 if(data.data[i].MobileNumber.startsWith('91'))
						 {
							...
							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="customer_name">'+data.data[i].BillDate+'</span></div>';
							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="customer_name">'+data.data[i].BillNumber+'</span></div>';
							
							str1 +='<div class="approval_block"><img src="assets/images/retailer_call_icon.png" class="location_icon"/><span class="approval_email"><a href="tel:'+checkforundefined(data.data[i].MobileNumber)+'" class="a_link_call_new">Mobileno: '+data.data[i].MobileNumber+'</a></span></div>';
							
							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">Amount: '+checkforundefined(data.data[i].BillAmount)+'</span></div>';
							...
							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">Rate: '+checkforundefined(data.data[i].rate)+'</span></div>';

							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">IHB MobileNo: '+checkforundefined(data.data[i].IHB_MobileNo)+'</span></div>';
							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">IHB Name: '+checkforundefined(data.data[i].IHB_Name)+'</span></div>';
							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">IHB Address: '+checkforundefined(data.data[i].IHB_Address)+'</span></div>';

							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">Store: '+checkforundefined(data.data[i].StoreName)+'</span></div>';
							
							str1+='<div>';
							str1+='<div class="ui-grid-a apr_block"><div class="ui-block-a"></div><div class="ui-block-b"><a href="javascript:gotoApprovalpage_txns(\''+data.data[i].BillNumber+'\',\''+data.data[i].MobileNumber+'\',\''+data.data[i].Quantity+'\');" class="btn_primary_approve btn_aaprov mt-20 ui-link">Approve/Reject</a></div></div>';
							str1+='</div>';
...
function gotoApprovalpage_txns(txn_id,cont_mobile,noofbags)
{
...
function uploadPhoto_shop(imageURI) 
{
				...
				shop_img="https://taghash.co/ACC/uploads/"+options.fileName+".jpg";
...
function uploadPhoto_pan(imageURI) 
{
				...
				pan_img="https://taghash.co/ACC/uploads/"+options.fileName+".jpg";
...
function capturePhoto_aadhar(imageURI) 
{
				...
				aadhar_img="https://taghash.co/ACC/uploads/"+options.fileName+".jpg";
...
function get_distributors_against_se()
{
	$.ajax({
		...
		timeout: 50000,
		dataType: 'json',
		data: {'apiuid': 'MLOYALAPI','apipswd':'Ml0yalAP!2o14','secode': store.storecode},
		complete: function(xhr, textStatus) {
	   //called when complete
		}, success: function(data, textStatus, xhr) {
			 ...
			 //var data = JSON.parse(data);
				...
				else
				{
					   if(data.data.length>=0)
					   {
						 for (var i = 0; i < data.data.length; i++)
						 {
							listItems+= "<option value='" + data.data[i].Distributor_Code + "'>"+data.data[i].Distributor_Name+"</option><hr>";
						 }
					   }
				}
...
function existingUser_check_byse1(str)
{
	console.log(str);
	document.getElementById("searchlist_div").innerHTML = '';
	$('#tx_smobile').val('');
	var sarchval = $('#'+str).val();
	console.log(sarchval)
	//  var searchcust = $('#tx_smobile').val();

	 if(sarchval=='')
	 {
		toast('Please Enter search values.');
	 }
	 else
	 {
	  
	  $.ajax({
	    url: SERVER2+'search_contractor_advance_api.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&searchval='+sarchval,
	    type: 'GET',
	    timeout: 50000,
		dataType: 'json',
		// data: {'apiuid':'MLOYALAPI','apipswd':'Ml0yalAP!2o14','searchval': sarchval},
		complete: function(xhr, textStatus) {
	    //called when complete
		},
		success: function(data, textStatus, xhr) {
		//    console.log('JSONDATA'+JSON.stringify(data));
		   ...
		   var datalist = data.data;
			...
			{
				if(checkforundefined(datalist[j].CustomerName).trim()!='')
				{
				listItems+='<p onclick=existingUser_check_byse(\''+datalist[j].mobileno+'\')>'+datalist[j].CustomerName+ ' ' +'('+ datalist[j].mobileno + ')' +'</p>';
...
function existingUser_check_byse(searchcust)
{
	 ...
	 else
	 {
	  ...
	  $.ajax({
		...
		//    console.log(data.data[0].CustomerName);
			fetchcurrent_Location();
			$('#tx_smobile_search').val(data.data[0].CustomerName);
			if(data.data.length>0)
			{
			
				
				if(data.data[0].point_claim=='dealer')
				{
					 $("#tform").css("display","block");
					 $('#tx_smobile').val(searchcust);
					 showIhbMemberList(searchcust);
				...
				else
				{
					...
					toast("Point clain can only be done through contractor.");
					$("#tform").css("display","none");
			...
			{
				toast("Not a registered contractor");
				$("#tform").css("display","none");
		...
		else
		{
		  $.ajax({
			...
			success: function(data, textStatus, xhr) {
				...
				if(data.indexOf("Success")>=0)
				{
					toast('Member profile has been updated successfully.');
	...
	$.ajax({
		...
		success: function(data, textStatus, xhr) {
			...
			{   
				...
				$('#update_mobile2_byse').val(data.data[0].altmobileno);
...
function capturePhoto_profile()
{
	...
	var mobileno = $('#update_mobile_de').val();
	if(mobileno=='')
	{
...
function capturePhoto_profile_old()
{
	  navigator.camera.getPicture(uploadPhoto4, onFail, { quality: 40,destinationType: navigator.camera.DestinationType.FILE_URI,targetWidth:512,targetWidth:512,correctOrientation:true});
...
function uploadPhoto4(imageURI) 
{
				...
				file_pphoto="https://taghash.co/ACC/uploads/"+options.fileName+".jpg";
				...
				ft.upload(imageURI, encodeURI("https://taghash.co/ACC/upload_photo_new.php"), win, failPhoto4, options, true);
}
...
function uploadPhoto_adhar1(imageURI) 
{
				...
				file_adimg_front="https://taghash.co/ACC/uploads/"+options.fileName+".jpg";
...
function uploadPhoto_adhar2(imageURI) 
{
				...
				file_adimg_back="https://taghash.co/ACC/uploads/"+options.fileName+".jpg";
				...
				ft.upload(imageURI, encodeURI("https://taghash.co/ACC/upload_photo_new1.php"), win, failAdhar2, options, true);
}
...
function site_Reg(xname,xmobile,btnID,materialtype)
{
 ...
 if(sname=='' || mobile=='' || material == undefined)
 {
     toast('Please enter mandatory fields!');
 }
 else
...
{
        var cin = $('#inputtxt').val();
        if(cin==''){
		 toast('Please enter SAP Code');
	    }
	    else
	    {
 ...
 }else
{
 ...
 $.ajax({
   url: SERVER2 + 'login_json_api.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&userid='+ uname + '&pswd='+ pwd,
   type: 'GET',
   timeout: 50000,
    dataType: 'json',
    // data: {'login_json_api.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&userid=' + uname + '&pswd='+ pwd},
    complete: function(xhr, textStatus) {
   //called when complete
    },
    success: function(data, textStatus, xhr) {
     ...
     console.log(JSON.stringify(data));
		...
		else
		{
			...
			//toast(data.data);
			// console.log(data.data[0].store_code);
			store.storecode = data.data[0].store_code ;
			store.store_name = data.data[0].store_name;
            localStorage.setItem('scode',store.storecode);
			//document.getElementById("user_id_se").innerHTML='SE Code '+store.storecode;
    		document.getElementById("user_id1_se").innerHTML='Dealer Code '+store.storecode;
			document.getElementById("user_name").innerHTML= store.store_name;
			document.getElementById("user_name_se1").innerHTML= store.store_name;
			...
			document.getElementById("sortname1").innerHTML= fn;
			document.getElementById("sortname2").innerHTML= fn;
			document.getElementById("sortname3").innerHTML= fn;
			document.getElementById("sortname4").innerHTML= fn;
			document.getElementById("sortname5").innerHTML= fn;
...
function capturePhoto_kycdoc_bydealer()
{
	...
	{
		navigator.camera.getPicture(uploadPhoto_kycdoc_bydealer, onFail, { quality: 40,destinationType: navigator.camera.DestinationType.FILE_URI,targetWidth:512,targetWidth:512,correctOrientation:true});
	}
...
function startcam()
{
		...
		var camera_button = document.getElementById("start-camera");
...
function capturePic()
{
	     ...
	     canvas.getContext('2d').drawImage(video, 0, 0, canvas.width, canvas.height);
            ...
            var image_base64 = document.querySelector("#canvas").toDataURL('image/jpeg').replace(/^data:image\/jpeg;base64,/, "");
	     ...
	     doc_img="https://taghash.co/ACC/uploads/"+fName+".jpg";
       ...
       $.ajax({ 
   		 type: "POST", 
    	 url: "https://taghash.co/ACC/uploadweb.php",
    	dataType: 'text',
    		data: {
        		image : image_base64,
        		name :  fName
...
function uploadPhoto_kycdoc_bydealer(imageURI) 
{

				var mobileno = $('#update_mobile_de').val();

				var b_Image = document.getElementById('kycdoc_photo_de');
				b_Image.style.display = 'block';
				b_Image.src = imageURI;
				...
				//var cattype = localStorage.getItem('cattype');
				statusDom=document.getElementById("kycdoc_photo_status_bydealer");
				statusDomdiv=document.getElementById("kycdoc_photo_status_div_bydealer");
				...
				ft.upload(imageURI, encodeURI("https://taghash.co/ACC/upload_photo_new.php"), win, failPhoto_kycdoc_bydealer, options, true);
}

function failPhoto_kycdoc_bydealer(error)
{
			toast('Photo upload failed. Please try after sometime: ' + error.code);
			localStorage.removeItem('IdProof_pic');
			doc_img = '';
			document.getElementById('kycdoc_photo_de').src = "assets/images/camera_icon.png";
}
...
function getProductList(srt)
{
	var showfilter = $('#tx_showproductfilter').val();
	console.log(showfilter);
	$.ajax({
		url: SERVER2+'get_itembrand_by_SUcode.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&sucode=',
		type: 'GET',
		timeout: 30000,
		success: function(data, textStatus, xhr) {
         ...
         console.log(data);
               ...
               str +='<div class="thumb_group flex-wrap">';
			   for(var i= 0; i < data.data.length; i++) 
			   {
				if(showfilter == data.data[i].MATERIAL_TYPE)
					{
				    var p_img1 = (data.data[i].image_Path.replace(/\s/g, '')).replace('.jpg','.png');
					var p_img = 'https://taghash.co/ACC/products/'+p_img1;
					//console.log('p_img:'+p_img);

					str +='<div class="thumb ui-link" href="#">';
					str +='<div class="thumb_icon test">';
					str +='<img src="'+p_img+'" alt="" class="image34">';
					str +='<div class="Rewards_text">'+data.data[i].item_brand+'</div>';
					str +='</div>';
					str +='<div class=" ui-radio"><input type="radio" name="tx_product" id="tx_product'+i+'" value="'+data.data[i].item_brand + '|' +data.data[i].Pckg_Type+'"></div>';
					str +='</div>';
					}
		 		}
		 		str +='</div>';

				 document.getElementById("productlist_div").innerHTML= str;

				 
	   }
    });

	if(showfilter == 'LEAK BLOCK')
	{
		$("#productliq_div").css('display', 'block');
		$("#productbag_div").css('display', 'none');
	}
  ...
  function showIhbMemberList(mobileno)
  {
	$.ajax({
		url:SERVER2+'get_ihbdetail_by_contractor.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&mobileno='+mobileno,
		type: 'GET',
		timeout: 50000,
		dataType: 'json',
	   ...
	   var data1 = data.sms;
			var listItems= "";
				listItems+= "<option value=''  disabled='disabled' style='background-color:#3E3E3E'  selected='selected'>Select Home Owner </option>";
			for (var i = 0; i < data1.length; i++)
			{
				// console.log(data[i].storename);
					listItems+= "<option value='" + data1[i].IHBMobileNo + "'>" + data1[i].IHBName + ' ' +'('+data1[i].IHBMobileNo+')' + "</option>";
			}

			listItems+="<option value='none'>Register a New site  </option>";
					  $("#tx_ihblist").html(listItems);
					  //checkIhbNumber("tx_ihblist");
		  },
		  error: function(xhr, textStatus, errorThrown) {
		}
	  });
...
function checkIhbNumber(str)
{
   ...
   if(ihbno!='none')
   {
	...
	// if(ihbno.trim()!='')
	// {
		$('#tx_ihbmobile').val(ihbno);
		if(ihbno.length == 10){
            // toast('correct mobile number');
			$.ajax({
				url: SERVER2+'get_ihb_customer_details.asp?apiuid=MLOYALAPI&apipswd=Ml0yalAP!2o14&mobileno='+ihbno,
				type: 'GET',
				timeout: 50000,
				dataType: 'json',
				data: { },
			   success: function(data, textStatus, xhr) {	
					//    console.log('JSONDATA:'+JSON.stringify(data));
					   console.log(data);
					   if(data.sms.length>=1){
					   $('#tx_ihbname').val(data.sms[0].IHBName);
					   $('#tx_address').val(data.sms[0].IHBAddress);
					   $('#tx_matrialstatus').val(data.sms[0].Materialstatus);
					   $('#tx_ihbpincode').val(data.sms[0].ZipCode);
					   document.getElementById('tx_ihbmobile').readOnly = true;
					   }
					   else
					   {
							//toast(data.error);
	...
	{
		document.getElementById('tx_ihbmobile').readOnly = false;
		$('#tx_ihbmobile').val('');
		$('#tx_ihbname').val('');
		$('#tx_address').val('');
		$('#tx_ihbpincode').val('');
...
function checkIhbNumber2(str)
{
	
	
	var ihbno_type = $('#tx_ihblist').val();
	var ihbno = $('#tx_ihbmobile').val();
	console.log(ihbno);
   if(ihbno_type=='none')
   {
	    

		check_is_registered_ihbsite(ihbno);
...
function check_is_registered_ihbsite(ihbmobile)
{
	...
	$.ajax({
			   ...
			   success: function(data, textStatus, xhr) {	
					   ...
					   {
							//toast('Site will be registered first before transaction');
							$('#site_new_popup').popup();
						    $('#site_new_popup').popup("open");
...
function close_site_new_popup()
{  
	$('#site_new_popup').popup("close");
}

function calculateMt()
{
	var bagcount = $('#tx_quantity').val();
	var rate = $('#tx_rate').val();

	if(bagcount.trim()=='' || bagcount=='0')
	{
		toast('Please check Bag Quantity');
		$('#tx_billamt').val('0');
	}
	// else if(rate.trim()=='' || rate=='0')
	// {
	// 	toast('Please check Price per unit');
	// 	$('#tx_billamt').val('0');
	// }
	else
	{
		var bval = parseFloat(parseInt(bagcount) * parseInt(rate)).toFixed(2);
		$('#tx_billamt').val(bval);
	}
}	

function checkapp_newsite_popup()
{
	...
	str +='</div>';
	str +='<div class="notification_pop">';
	str +='<a class="notification_close" onclick="javascript:closePop();">+</a>';
	str +='<div class="notification_block">';
	str +='<p class="notification_msg">Site will be registered first before transaction</p>';

	str +='<div class="refer_btns_version">';
	str +='<button onclick="javascript:closePop();" style="margin-right:4%;" data-role="none">Cancel</button>';
...
{
	  $.ajax({
		url: SERVER2+'get_associated_member_by_store_json_api.asp',
		type: 'GET',
		timeout: 50000,
		dataType: 'json',
		data: {'apiuid': 'MLOYALAPI','apipswd':'Ml0yalAP!2o14','store_code': store.storecode },
		complete: function(xhr, textStatus) {
	   //called when complete
		}, success: function(data, textStatus, xhr) {
			...
			else
			{
				 for (var i = 0; i < data.data.length; i++)
				 {
				   ...
				   if(showTDataless < data.data[i].sno && showMatData >= data.data[i].sno){
						...
						str1 +='<div class="approval_block"><img src="assets/images/retailer_name_icon.png" class="location_icon"/><span class="customer_name">'+data.data[i].FirstName+' '+data.data[i].LastName+'</span></div>';
						
						
						str1 +='<div class="approval_block"><img src="assets/images/retailer_call_icon.png" class="location_icon"/><span class="approval_email"><a href="tel:'+checkforundefined(data.data[i].Mobileno)+'" class="a_link_call_new"  style="font-weight: 500;">Mobileno: '+data.data[i].Mobileno+'</a></span></div>';
						str1 +='<div class="approval_block"><img src="assets/images/retailer_mail_icon.png" class="location_icon"/><span class="approval_email">Member Id: '+checkforundefined(data.data[i].memberid)+'</span></div>';

					
                        // str1 +='<div class="approval_block"><img src="assets/images/date.png" class="location_icon"/><span class="approval_email">DOB: '+checkforundefined(dob_se1)+'</span></div>';
						// }

						var is_approved ='';
						if(checkforundefined(data.data[i].SE_IsApproved)!='')
						{
							is_approved = checkforundefined(data.data[i].SE_IsApproved);
						}
						else
						{
							is_approved ='Pending';
				...
				//  str1 +='<div><a onclick="tranSubmore()" style="margin-left: 10px;" >Back...</a><a onclick="tranAddmore()" style="margin-left: 50%;">Next...</a></div>';
				 
				 var metedataless = localStorage.getItem('showTDataless');
				  if(metedataless == '0'){
...
function tranAddmore(){
	...
	localStorage.setItem('showTDataless',tranFirstShow1);
	localStorage.setItem('tranFirstShow',tranFirstShow);
	viewContractors();
}

function tranSubmore(){
	var metadataL = localStorage.getItem('showTDataless');
...
function viewTransactions()
{
	  ...
	  $.ajax({
			url: SERVER2+"get_dealer_transaction_details_json_api.asp?apiuid=MLOYALAPIACC&apipswd=Ml0yalAP!2o14ACC",
		    type: 'Get',
		    timeout: 50000,
		    dataType: 'json',
		    data:{
				...
				//  console.log("data:"+JSON.stringify(data));
				 ...
				 else
				 {
					 for (var i = 0; i < data.data.length; i++)
					 {
						 ...
						 if(data.data[i].Member_Mobile.startsWith('91'))
						 {
							data.data[i].Member_Mobile=data.data[i].Member_Mobile.substr(2,data.data[i].Member_Mobile.length);
						 }
						 var historycountdata = localStorage.getItem('historyFirstShow');
						 // console.log(historycountdata);
						 if(historycountdata <= 3){
							 historyFirstShow = 3;
							 // console.log('in if condition');
						 }else{
							 historyFirstShow = localStorage.getItem('historyFirstShow');
							 // console.log('in else condition');
						 }
						 localStorage.setItem('historyFirstShow',historyFirstShow);
						 var showMatData = localStorage.getItem('historyFirstShow');
						 var showTDataless = localStorage.getItem('showYDataless');
						 if(showTDataless < 0){
							var val = 0;
							localStorage.setItem('showYDataless',val);
						}
						if(showTDataless < data.data[i].sno && showMatData >= data.data[i].sno){
							str1 +='<div class="primary_block">';
							str1 +='<div class="ui-grid-solo">';
							...
							str1 +='<div class="approval_block"><img src="assets/images/date.png" class="location_icon"/><span class="customer_name">'+data.data[i].Created_On+'</span></div>';
							// str1 +='<div class="approval_block"><img src="assets/images/bill.png" class="location_icon"/><span class="customer_name">'+data.data[i].bill_number+'</span></div>';
							
							str1 +='<div class="approval_block"><img src="assets/images/retailer_call_icon.png" class="location_icon"/><span class="approval_email"><a href="tel:'+checkforundefined(data.data[i].Member_Mobile)+'" class="a_link_call_new" style="font-weight:500;">Mobile No: '+data.data[i].Member_Mobile+'</a></span></div>';
							
							str1 +='<div class="approval_block"><img src="assets/images/retailer_name_icon.png" class="location_icon"/><span class="approval_email">Name: '+checkforundefined(data.data[i].Member_Name)+'</span></div>';
							// str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">Quantity: '+checkforundefined(data.data[i].Quantity)+'</span></div>';
							// str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">Rate: '+checkforundefined(data.data[i].rate)+'</span></div>';

							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">Member Id: '+checkforundefined(data.data[i].memberid)+'</span></div>';
							str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">Earn Points: '+checkforundefined(data.data[i].Points)+'</span></div>';
							// str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">IHB Address: '+checkforundefined(data.data[i].IHB_Address)+'</span></div>';

							// str1 +='<div class="approval_block"><img src="assets/images/retailer_license_icon.png" class="location_icon"/><span class="approval_email">Store: '+checkforundefined(data.data[i].StoreName)+'</span></div>';
							
							str1+='<div>';
							//str1+='<div class="ui-grid-a apr_block"><div class="ui-block-a"></div><div class="ui-block-b"><a href="javascript:gotoApprovalpage_txns(\''+data.data[i].BillNumber+'\',\''+data.data[i].MobileNumber+'\',\''+data.data[i].Quantity+'\',\''+data.data[i].IHB_MobileNo+'\');" class="btn_primary_approve btn_aaprov mt-20 ui-link">Approve/Reject</a></div></div>';
							str1+='</div>';
					...
					//  str1 +='<div><a onclick="tranHSubmore()"  style="margin-left: 10px;">Back...</a><a onclick="tranHAddmore()" style="margin-left: 50%;">Next...</a></div>';
				 
					 var showDataless = localStorage.getItem('showYDataless');
					 if(showDataless == '0')
					 {
						 str1 +='<div><a onclick="tranHSubmore()" id="hideback" style="margin-left: 10px;display:none;">Back...</a><a onclick="tranHAddmore()"   >Next...</a></div>';
					 }
					 else
					 {
						 str1 +='<div class="ne"><a onclick="tranHSubmore()" id="hideback" style="margin-left: 10px;">Back...</a><a onclick="tranHAddmore()"   >Next...</a></div>';
					 }
...
function tranHAddmore(){
	var metedata = localStorage.getItem('historyFirstShow');
	historyFirstShow = parseInt(metedata) + parseInt(3);
	historyFirstShow1 = parseInt(historyFirstShow) - parseInt(3);
	localStorage.setItem('showYDataless',historyFirstShow1);
	localStorage.setItem('historyFirstShow',historyFirstShow);
	viewTransactions();
}

function tranHSubmore(){
	var metedataL = localStorage.getItem('showYDataless');
	if(metedataL <= 0){
		console.log('NO');
	}else{
	var metedata = localStorage.getItem('historyFirstShow');
...
function toggle_rcountDivs()
{
	...
	if (rcount=='3')
	{
		$("#update_runningsite_div1").css("display","block");
		$("#update_runningsite_div2").css("display","block");
		$("#update_runningsite_div3").css("display","block");
		$("#update_runningsite_div4").css("display","none");
		$("#update_runningsite_div5").css("display","none");
	...
	if (rcount=='4')
	{
		...
		$("#update_runningsite_div2").css("display","block");
		$("#update_runningsite_div3").css("display","block");
		$("#update_runningsite_div4").css("display","block");
		$("#update_runningsite_div5").css("display","none");
	...
	if (rcount=='5')
	{
		...
		$("#update_runningsite_div3").css("display","block");
		$("#update_runningsite_div4").css("display","block");
		$("#update_runningsite_div5").css("display","block");
	}
}
